...
public class RabbitMQConfig {
    ...
    @Bean
    public Binding bindingNovoUsuario(Queue queueNovoUsuario, TopicExchange caronaExchange) {
        return BindingBuilder.bind(queueNovoUsuario).to(caronaExchange).with(ROUTING_KEY_NOVO_USUARIO);
    }

    @Bean
    public Binding bindingViagemCriada(Queue queueViagemCriada, TopicExchange caronaExchange) {
        return BindingBuilder.bind(queueViagemCriada).to(caronaExchange).with(ROUTING_KEY_VIAGEM_CRIADA);
    }

    @Bean
    public Binding bindingViagemPassageiro(Queue queueViagemPassageiro, TopicExchange caronaExchange) {
        return BindingBuilder.bind(queueViagemPassageiro).to(caronaExchange).with(ROUTING_KEY_VIAGEM_PASSAGEIRO);
    }
